# Source: https://raw.githubusercontent.com/Tarpelite/UniNLP/master/examples/run_parsing_v2.py
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results, preds_label_list, preds_arc_list = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev")
...
def las_score(true_label_list, true_heads_list, predict_label_list, predict_heads_list):
    ...
    for true_labels, true_heads, predict_labels, predict_heads in zip(true_label_list, true_heads_list,
                                                                      predict_label_list, predict_heads_list):
        hit = 0
        for true_label, true_head, predict_label, predict_head in zip(true_labels, true_heads, predict_labels,
                                                                      predict_heads):
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    return results, preds_label_list, preds_arc_list
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint,
                                                ...
                                                cache_dir=None)
            model.to(args.device)
            result, preds_label_list, preds_arc_list = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev", prefix=global_step)
            if global_step:
