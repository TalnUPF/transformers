# Source: https://raw.githubusercontent.com/Tarpelite/UniNLP/master/examples/utils_parsing_v2.py
...
def read_examples_from_file(file_path, mode):
    ...
    with open(file_path, encoding="utf-8") as f:
        ...
        for tokenlist in parse_incr(f):
            ...
            # remove sentences containing tokens with '.' on id  # TODO lpmayos document better
            ...
            if len(drop_positions) == 0:
                ...
                examples.append(InputExample(guid="%s-%d".format(mode, guid_index), words=words, heads=heads, labels=labels))
                guid_index += 1
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, head, label in zip(example.words, example.heads, example.labels):
            if ':' in label:
                old_label = label
                label = label.split(':')[0]
                logger.info('Label %s contains language specific relations; we convert it to %s' % (old_label, label))
    ...
    logger.info("*** max_len:{}  min_len:{} avg_len:{}***".format(max(cnt_counts), min(cnt_counts),
                                                                  sum(cnt_counts) / len(cnt_counts)))
...
def get_labels(path):
    if path and os.path.exists(path):
        ...
        return ['acl', 'advcl', 'advmod', 'amod', 'appos', 'aux', 'case', 'cc', 'ccomp', 'clf', 'compound', 'conj', 'cop', 'csubj', 'dep', 'det', 'discourse', 'dislocated', 'expl', 'fixed', 'flat', 'goeswith', 'iobj', 'list', 'mark', 'nmod', 'nsubj', 'nummod', 'obj', 'obl', 'orphan', 'parataxis', 'punct', 'ref', 'reparandum', 'root', 'vocative', 'xcomp']